...
function generateFibonacciResources() {
  ...
  const selectedFibonacci = new Set();
  while (selectedFibonacci.size < 3) {
    selectedFibonacci.add(fibonacciNumbers[Math.floor(Math.random() * fibonacciNumbers.length)]);
  }

  const randomNumbers = new Set();
  while (randomNumbers.size < 4) {
    const randomNum = Math.floor(Math.random() * (100 - 10 + 1)) + 10;
    if (!selectedFibonacci.has(randomNum)) {
      randomNumbers.add(randomNum);
    }
  }

  return Array.from(selectedFibonacci).concat(Array.from(randomNumbers)).sort(() => 0.5 - Math.random());
}

function generateOddTownResources() {
  const oddNumbers = new Set();
  while (oddNumbers.size < 3) {
    oddNumbers.add(Math.floor(Math.random() * (10 - 4 + 1)) + 4);
  }
  return Array.from(oddNumbers);
}
</script>
